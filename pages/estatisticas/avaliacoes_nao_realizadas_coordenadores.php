...
<body>
  ...
  <a href="avaliacoes_usuarios_coordenadores.php?id_tipo=null&idusuario=null&id_data=null" class="btn btn-outline-secondary btn-lg m-2">Editor e Avaliação Por Usuário</a>
  <a href="avaliacoes_nao_realizadas_coordenadores.php?id_data=6&id_data=null&modo=null" class="btn btn-secondary btn-lg m-2">Usuários Pendentes</a>
  </div>
...
if ($_GET['modo'] == 1) {
  $modo = 'NOT';
  $titulo = "<p class='fw-bold h4 pt-2 pb-2' >Auto-Avaliação Não Realizada:</p>";
} if ($_GET['modo'] == 2) {
  $modo = ' ';
  $titulo = "<p class='fw-bold h4 pt-2 pb-2' >Auto-Avaliação Realizadas:</p>";
}
...
AND $modo EXISTS (
  SELECT 1 
  FROM avaliacoes a 
WHERE e.idusuario = a.idusuario and data = $ano_avaliacao and lidersupervisor = 0)
and lider = 0
AND e.avaliador <> 0
AND avaliador = $id_login
