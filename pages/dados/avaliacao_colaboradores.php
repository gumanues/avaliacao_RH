...
<!-- Envios -->
  ...
  <div>
    ...
    <p class="text-body-secondary d-flex justify-content-center py-1">Colaborador se auto-avalia</p>
  </div>

  <div class="container py-2">
    <form action="../../funcoes/envio_avaliacao_colaborador.php" method="post">
      <div class="mb-3">
        <label for="idusuario" class="form-label pt-3">Selecione seu Nome:</label>
        <select class="form-select" name="idusuario" id="idusuario" required>
          ...
          WHERE situacao = 1
            AND ((
              (e.lider = 0 AND NOT EXISTS (
                SELECT 1
                FROM avaliacoes a
                WHERE e.idusuario = a.idusuario AND data = $ano_vigente and lidersupervisor = 0
              ))) 

             or ((
              ((e.lider = 2) AND NOT EXISTS (
                SELECT 1
                FROM avaliacoes a
                WHERE e.idusuario = a.idusuario AND data = $ano_vigente and idsupervisor = idusuario
              ))) and idusuario in($concede_usuario)))
              

          ORDER BY e.nomecompleto ASC
          
          ";
          // Parametrizado pelo IDussuario do banco de dados
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            <button type="button" class="btn botao btn-secondary" data-bs-dismiss="modal">Fechar</button>
          </div>
        </div>
      </div>
    </div>
